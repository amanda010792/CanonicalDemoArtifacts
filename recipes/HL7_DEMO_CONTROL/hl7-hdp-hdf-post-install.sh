...
deployTemplateToNifi () {
        ...
        cd $ROOT_PATH
        wget https://github.com/hortonworks/SE-demo/blob/master/DoctorsNotes/other/Send_HL7_Messages_to_Solr%20(NiFi).xml
        chmod 777 $ROOT_PATH/Send_HL7_Messages_to_Solr%20(NiFi).xml
        sleep 60
        
    TEMPLATEID=$(curl -v -F template=@"$ROOT_PATH/Send_HL7_Messages_to_Solr%20(NiFi).xml" -X POST http://$AMBARI_HOST:9090/nifi-api/process-groups/root/templates/upload | grep -Po '<id>([a-z0-9-]+)' | grep -Po '>([a-z0-9-]+)' | grep -Po '([a-z0-9-]+)')
        sleep 60
